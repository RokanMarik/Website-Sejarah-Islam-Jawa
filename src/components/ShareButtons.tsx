...
export default function ShareButtons({ title, slug }: ShareButtonsProps) {
  const baseUrl = typeof window !== "undefined" ? window.location.origin : "https://nusahistoria.com";
  ...
  return (
    <div className="mt-8 pt-6 border-t border-gray-800">
      <h4 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Bagikan Artikel</h4>
      <div className="flex flex-wrap gap-3">
        {shareLinks.map((link) => (
          <a
            ...
            className={`flex items-center gap-2 px-4 py-2 border border-gray-700 rounded-full text-gray-400 transition-all ${link.color}`}
        ...
        >
          ...
          <span className="text-xs uppercase tracking-wider hidden sm:inline">Salin</span>
        </button>
      </div>
    </div>
  );
